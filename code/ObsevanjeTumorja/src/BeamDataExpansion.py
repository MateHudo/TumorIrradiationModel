...
def repair_wrong_expansion(matrix):
    ...
    # repaired = np.zeros((num_beams, N, N), dtype=matrix.dtype)
    # # first N beams are correct, also second N beams and last N beams
    # repaired[:N] = matrix
    # repaired[N:2*N] = matrix[N:2*N]
    # repaired[2*N:3*N] = matrix[2*N:3*N]
